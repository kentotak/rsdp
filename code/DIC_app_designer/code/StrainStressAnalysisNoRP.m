function StrainStressAnalysisNoRP(final_plot,Width,Thickness,Samplename)
close;% all;
...
Data_value_begin = {Samplename; Width; Thickness; Egiga;Stress_import(fracture_index); ...
    ...
    maxstress_ini; []}; 
% NO_RP
%(Strain_corr(find(Stress_import==maxstress_ini)) - maxstress_ini/E)};
%            DATAVALUE1TEMP ={};
%            DATAVALUE2TEMP ={};     
%     for j=1:length(RP0X)
...
%         clear DATAVALUE1TEMP DATAVALUE2TEMP;
        ...
        Data_valueRP = [cell(length(RP0X),1);cell(length(RP0X),1)];
...
%         end
        if fracture_point_selected == 1
            ...
            legend('Raw data','Elastic behavior','Max stress','Fracture point','Location','NorthEastOutside')
        else
            % NO_RP
            %legend('Raw data','Elastic behavior','Max stress','RP_0X(s)','Location','NorthEastOutside')
            legend('Raw data','Elastic behavior','Max stress','Location','NorthEastOutside')
        end
